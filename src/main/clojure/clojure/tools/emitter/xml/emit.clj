...
(ns clojure.tools.emitter.xml.emit
  ...
  (if (and (primitive? tag)
    ...
    (when (primitive? box)
      (let [method (if (and (numeric? box) (or unchecked? *unchecked-math*))
                     (str "unchecked" (str/capitalize (.getName ^Class box)) "Cast")
...
(defn emit-xml
  ...
  ([ast opts]
     (binding [*classes* (atom {:classes []
                                :ids     #{}})]
       (let [x (emit ast opts)]
         (println "emit-xml result: " (type x))
         x)
       #_(:classes @*classes*))))
...
(defmethod -emit :const
  ...
  ^:const
  (println "-EMIT :CONST: " ast)
  (xml/element :const {:val (:val ast)})
  #_[(case val
     (true false)
     (if (primitive? tag)
       [:push val]
...
(defn invoke->meta [ast]
  `{:line ~(str (-> ast :meta :line))
   :column ~(str (-> ast :meta :column))
   }
  )

;; (defn- dispatch->attribs [_ data] (type data))
(defn- dispatch->elts [_ data] (type data))

;; (defmulti ->attribs #'dispatch->attribs)
(defmulti ->elts #'dispatch->elts)

(defn map->attribs [coll]
  (println "MAP->ATTRIBS: " coll)
  (into {} (for [[k v] coll]
             (let [ks (name k)]
               [(if (str/ends-with? ks "?")
                  (subs ks 0 (- (count ks) 1))
                  k)
                (cond
                  (map? v) (if (empty? v) "" (map->attribs v))
                  (vector? v) "VECTOR"
                  (list? v) "LIST"
                  (var? v) "VAR"
                  (fn? v) "FN"
                  (instance? clojure.lang.Namespace v) "NS"
                  :else (str v))]))))

(defmethod ->elts clojure.lang.Fn ;; not IFn
  [key m]
  (println "->ELTS :Fn " key)
  (println "->ELTS :Fn " m)
  (xml/element key {} (str m)))

(defmethod ->elts clojure.lang.IPersistentList
  [key m]
  (println "->ELTS :IPersistentList " key)
  (xml/element key {} (str m)))

(defmethod ->elts clojure.lang.IPersistentMap
  [key m]
  (println "->ELTS:IPersistentMap " key)
  (let [atomics (filter #(and (not (coll? (second %)))
                              ;; (not (fn? (second %)))
                              (not (= java.lang.Object (second %))))
                        m)
        _ (println "ATOMICS: " (for [[k v] atomics] (str k)))
        _ (println "ATOMICS vs: " (for [[k v] atomics] (str v)))
        atomic-attribs (into {} (for [[k v] atomics]
                                  (let [ks (name k)]
                                    [(if (str/ends-with? ks "?")
                                       (subs ks 0 (- (count ks) 1))
                                       k)
                                     (str (cond
                                            ...
                                            (var? v) "VAR"
                                            :else (str v)))])))
        _ (println "ATOMIC-ATTRIBS: " atomic-attribs)
        colls (filter #(coll? (second %)) m)
        _ (println "COLLS: " (for [[k v] colls] (str k)))
        ]
    (xml/element key atomic-attribs
                 (if (empty? colls)
                   nil
                   (into []
                         (for [coll colls]
                           (do (println "Coll key: " coll)
                               (->elts (first coll) (second coll))))))
                 )))

(defmethod ->elts clojure.lang.IPersistentVector
  [key op-maps]
  "v is vector of maps?"
  (println "->ELTS :IPersistentVector " key)
  (xml/element
   key {}
   (for [op-map op-maps]
     (let [_ (println "OP-MAP: " op-map)
           elt-tag (:op op-map)
           atomics (filter #(and (not (coll? (second %)))
                                 ;;(not (fn? (second %)))
                                 (not (= java.lang.Object (second %))))
                           op-map)
           _ (println "ATOMICS: " (for [[k v] atomics] (str k)))
                                     ...
                                     (let [ks (name k)]
                                       [(if (str/ends-with? ks "?")
                                        ...
                                        (str (cond
                                               ...
                                               :else (str v)))])))
           _ (println "ATOMIC-ATTRIBS: " atomic-attribs)
           colls (filter #(coll? (second %)) op-map)
           _ (println "COLLS: " colls)
           ]
       (xml/element elt-tag atomic-attribs
                    (if (empty? colls)
                      nil
                      (into []
                            (for [coll colls]
...
(defn invoke->children [ast]
  (println "INVOKE->CHILDREN: " ast)
  (if (empty? ast)
    []
    (let [children-keys (:children ast)]
      (println "CHILDREN-KEYS " children-keys)
      (into [] (for [child-key children-keys]
                 (do
                   (println "CHILD-KEY: " child-key)
                   (let [child-val (child-key ast)]
                     (println "CHILD-VAL: " child-val)
                     (if (map? child-val)
                       (->elts child-key child-val)
                       (if (vector? child-val)
                         (->elts child-key child-val)
                         "UNEXPECTED")))))))))

(defn invoke->env [ast]
  (let [entries (into {} (for [[k v] (:env ast)]
                           [k
                            (cond
                              (map? v) (if (empty? v) "" (map->attribs v))
                              (vector? v) "VECTOR"
                              ...
                              (var? v) "VAR"
                              :else (str v))]))]
  (xml/element :env entries)))

(defmethod -emit :invoke
  [{:keys [fn args env to-clear?] :as astx} frame]
  (println "-EMIT :INVOKE" astx)
  ;; (let [ast (walk/prewalk #(if (map? %)
  ;;                            ;; these are java-specific kws?
  ;;                            (dissoc % :o-tag :inline :inline-arities)
  ;;                            %)
  ;;                         astx)
  (let [ast (walk/prewalk-replace {:o-tag ::jvm/o-tag
                                   :inline ::jvm/inline
                                   :inline-arities ::jvm/inline-arities}
                                  astx)
        x (xml/element :invoke {:form (str (:form ast))
                                :top-level (str (:top-level ast))
                                :op (str (:op ast))
                                }
                       (xml/element :meta (invoke->meta ast))
                       (invoke->env ast)
                       (invoke->children ast)
                       )]
    (println "X: " (type x) " : " x)
...
(defn emit-constants [{:keys [class constants]}]
  "arg constants is a map whose keys and vals are maps"
  (println "EMIT-CONSTANTS " constants)
  (let [res (mapcat (fn [{:keys [val id tag type]}]
                      (xml/element :const {:id id :val val :tag tag :type type}))
                    (vals constants))]
    (println "XML res: " res)
    res))
